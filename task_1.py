...
# Ваш уровень: 2
exp = int(input('Введите полученные очки опыта: '))
level = 1
if exp >= 1000 and not(exp >= 2500):
  level += 1
  print('Поздравляем! Вы перешели на уровень: ', level)
elif exp >= 2500 and not(exp >= 5000):
  level += 2
  print('Поздравляем! Вы перешели на уровень: ', level)
elif exp >= 5000:
  level += 3
  ...
  print('Вы еще не набрали необходимое кол-во опыта для повышения уровня.')